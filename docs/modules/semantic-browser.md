...
- **概览**：业务定义、所属域、核心属性和使用约束。
- **定义**：按对象类型展示的公共属性、Python 符号和定义位置。
...
打开、切换项目和手动刷新时读取 Catalog；搜索、筛选、详情及关系图操作只使用当前快照。没有后台轮询、文件监听
...
Host 连接重置清除绑定数据，重新选择项目。Workspace 删除后不再提供旧项目内容。
