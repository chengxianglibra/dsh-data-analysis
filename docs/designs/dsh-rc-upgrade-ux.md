...
状态：S1 rc.1 基线已于 2026-09-11 完成，验收见[验证指南](../validation.md#dsh-rc1-基线验收)。S2 普通文件交付已于同日完成，验收见[普通文件交付验收](../validation.md#普通文件原生交付验收)；S3 空白首页快捷入口已实现并通过隔离 Web 验收，见[首页入口验收](../validation.md#空白首页工作区快捷入口验收)。编写日期：2026-09-11。

目标是利用 DeepSeek Harness `0.1.5-rc.1` 的公开能力，完善普通分析文件交付，改善已有分析入口的可发现性，并保持报告、引用和执行的身份边界。

首期包含三个依次验收的步骤：兼容适配、普通文件原生交付、空白首页快捷入口。全局分析面板、自定义 CSV 预览和 Agent Teams 仅记录后续方向，不纳入首期实现。
...
本次可利用的增量包括 `present` 显式文件交付、可扩展文档预览、文件原生打开动作、右侧栏交互完善以及全局面板扩展点。首页 `conversation.input.dock` 属于已有公开接缝，补充快捷入口不能归因于 rc.1 新增 API。
...
在 `conversation.input.dock` 注册一组“数据源与凭据”“语义层”“报告”按钮，位于原生输入框上方。
使用 Harness 公开 `conversationPhase` 和响应式 Conversation 状态，仅在 blank 阶段显示；正式对话继续使用现有顶栏入口。
该方案替换原右侧 guide 方案，不新增全局面板、工作区选择器或 guide 项。

用户选择 Workspace 后接受 Harness 连接空白 Session 的原生行为，不要求先发消息。
完全没有 Session 时不显示；插件不主动创建 Session。按钮从所属 Session 的 Workspace 成员关系绑定身份，
连接或 Workspace 状态未就绪时禁用，不借用最近访问的工作区。

首页与顶栏共用目录打开动作，点击时重新核验当前 Session、Workspace 和连接，再打开现有目录 Tab，
由 Harness 负责去重和页面生命周期。失败在首页入口旁显示错误；不读取或改写 composer 草稿、附件和引用。

图标和文案复用现有资源，订阅 Host locale；入口组与首页工作区标签、输入框左边缘对齐，窄屏保留文字并允许换行。仅显示入口不读取目录、
不初始化 Runtime、不读取凭据或执行分析。注册随插件卸载清理，报告正文语言保持不变。
...
| S3：首页快捷入口 | input dock 注册、共用导航、locale、相关测试 | 空白阶段显示、首轮后隐藏；真实点击正确目录；与顶栏去重；窄屏、草稿保留和无就绪 Workspace |
...
| 首页／顶栏入口交替点击 | 在 Host 规定的目标 pane 内复用同一目录；正确绑定所属 Workspace |
...
首期完成意味着 rc.1 候选通过规定验证，普通文件具备原生交付闭环，三个分析目录可从空白首页正确打开。发布、重装用户 profile、重启当前服务、清理状态及未来能力不属于该完成条件。升级失败时保留证据并修复候选，不通过降级或删除用户 Session 来规避问题。
