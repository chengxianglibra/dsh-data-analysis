...
export const REPORT_RENDERER_VERSION = 'dsh-data-analysis-html/v5' as const

const CSP =
  "default-src 'none'; style-src 'unsafe-inline'; img-src data:; font-src 'none'; script-src 'none'; object-src 'none'; frame-src 'none'; base-uri 'none'; form-action 'none'"
  ...
  const statements =
    findings.length === 0
      ...
      : `<ol class="evidence-list">${findings
          ...
          .join('')}</ol>`
  const provenanceLink = `<p class="source-provenance"><a href="#report-provenance">${copy.inventory}</a></p>`
...
function footer(report: CompiledReport, generatedAt: string): string {
  ...
  return `<footer><div class="footer-meta"><p><strong>${copy.generated}</strong> <time datetime="${escapeHtml(generatedAt)}">${escapeHtml(visibleTime)}</time></p><p class="freshness">${copy.freshness}</p></div><details class="provenance-index" id="report-provenance"><summary>${copy.inventory} · ${count}</summary><p class="support-boundary">${copy.support}</p><div class="footer-grid"><section><h3>${copy.artifacts}</h3>${artifacts}</section><section><h3>${copy.findings}</h3>${findings}</section></div></details></footer>`
}
