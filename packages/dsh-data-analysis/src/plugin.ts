...
export const MARIVO_REPORT_RENDERING_PROMPT = [
  ...
  'Finding IDs are optional source metadata. Attach the smallest sufficient non-redundant set only when exact persisted Finding provenance or an in-body evidence block is needed; ordinary blocks do not need Finding IDs solely for display.',
  'Prefer the most direct exact Finding for a scalar fact, and do not also attach a summary Finding that merely restates the same user-facing fact. Do not add a duplicate Evidence appendix or evidence block unless the user explicitly asks for a source inventory.',
