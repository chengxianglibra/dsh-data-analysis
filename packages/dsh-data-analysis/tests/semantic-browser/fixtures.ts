...
user_day = ms.time_dimension_column(name="user_day", entity=users, column="day", granularity="day")
user_spend = ms.measure_column(name="user_spend", entity=users, column="spend", additivity="additive")
user_revenue = ms.aggregate(name="user_revenue", measure=user_spend, agg="sum")
all_spend = ms.cumulative(name="all_spend", base=user_revenue)
...
stock = ms.measure_column(name="stock", entity=orders, column="stock", additivity=ms.semi_additive(over=ordered_at, fold=("percentile", 0.95)))
stock_total = ms.aggregate(name="stock_total", measure=stock, agg="sum")
stock_last = ms.aggregate(name="stock_last", measure=stock, agg="sum", fold="last")
stock_linear = ms.linear(name="stock_linear", add=[stock_last, stock_last])
