...
test('v2 compatibility manifest binds plugin, DSH, Marivo, and project contracts', () => {
  assert.equal(PLUGIN_VERSION, '0.2.2-dev.0')
  ...
  assert.equal(DSH_PEER_RANGE, '>=0.1.5-rc.1')
  assert.equal(MARIVO_VERSION, '0.5.5')
  assert.equal(MARIVO_PACKAGE_SPEC, 'marivo[duckdb,trino,clickhouse]==0.5.5')
