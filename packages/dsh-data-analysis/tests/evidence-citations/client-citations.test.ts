...
interface ClientExports {
  parseMarivoFootnotes(text: string): {
    references: string[]
...
test('Markdown scanner finds ordered unique references and definitions', async () => {
  ...
  assert.deepEqual(plain(parsed), {
    references: ['F2', 'F1'],
    definitions: [
      { handle: 'F1', body: 'first' },
      { handle: 'F2', body: 'second' },
    ],
  })
})
...
test('Markdown scanner ignores escaped tokens and inline or fenced code', async () => {
  ...
  assert.deepEqual(plain(parsed), {
    references: ['F4'],
    definitions: [{ handle: 'F4', body: 'visible definition' }],
  })
  assert.deepEqual(
    plain(client.parseMarivoFootnotes('`multiline\n[^mv-f5] code` visible[^mv-f6]')),
    { references: ['F6'], definitions: [] },
  )
  assert.deepEqual(plain(client.parseMarivoFootnotes('unmatched ` then visible[^mv-f7]')), {
    references: ['F7'],
    definitions: [],
  })
  assert.deepEqual(
    plain(
      client.parseMarivoFootnotes(
        ['`multiline', '[^mv-f8]: definition inside code', '` visible[^mv-f8]'].join('\n'),
      ),
    ),
...
test('registry metadata rejects malformed, duplicate, and out-of-range handles', async () => {
  ...
  assert.equal(
    client.parseCitationRegistryMeta({ ...meta([source('F1')]), dshSessionId: '' }),
    null,
  )
  ...
  assert.equal(
    client.parseCitationRegistryMeta(
      meta([{ ...source('F1'), rendered: { en: 'x'.repeat(8_193), zh: '事实' } }]),
    ),
    null,
  )
  assert.equal(
    client.parseCitationRegistryMeta(
      meta([{ ...source('F1'), rendered: { en: 'fact', zh: '值'.repeat(2_731) } }]),
    ),
    null,
  )
})
...
test('answer Definition resolves against the nearest complete registry and publishes Turn data', async () => {
  ...
  const state = client.marivoAnswerCitationsDefinition.start(
    {},
    { event },
    {
  ...
  assert.deepEqual(plain(state.citations), [
    {
      handle: 'F1',
      source: source('F1'),
      definitionStatus: 'matched',
      language: 'zh',
      statement: '指标 finding-F1：观测值为 12。',
    },
    {
      handle: 'F2',
      source: source('F2'),
      definitionStatus: 'matched',
      language: 'zh',
      statement: '指标 finding-F2：观测值为 12。',
    },
  ])
...
test('assistant Markdown is scanned per text block without cross-block definitions or fences', async () => {
  ...
  const state = client.marivoAnswerCitationsDefinition.start(
    {},
    { event: separateDefinition },
    {
      previous() {
        return { state: { registry: [source('F1')] } }
      },
    },
  )
  assert.deepEqual(plain(state.citations), [
  ...
  const fencedState = client.marivoAnswerCitationsDefinition.start(
    {},
    { event: fencedThenVisible },
    {
      previous() {
        return { state: { registry: [source('F1')] } }
      },
...
test('source card keeps the fact primary and machine metadata inside folded audit details', async () => {
  ...
  const tree = client.MarivoSourceCard({
    matched: [citation],
    t(key, values) {
      return values === undefined ? key : `${key}:${Object.values(values).join(':')}`
    },
  })
...
test('tool-calling assistant steps and answers without references produce no citation UI', async () => {
  ...
  toolCalling.data.message.content.push({
    type: 'tool-call',
    id: 'call',
    name: 'x',
    arguments: '{}',
