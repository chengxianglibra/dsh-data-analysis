...
test('Session traces require Frame previews while the Checker ignores Query payloads', async (t) => {
  const root = await workspace(t)
  const fixtureRoot = reportContractRoot()
  const artifact = JSON.parse(
    await readFile(new URL('fixtures/artifact-dataset.json', fixtureRoot), 'utf8'),
  ...
  const variants = [
    ...
    [{}],
    [query, query],
    [{ ...query, run_id: 'missing-run', output_artifact_ref: 'missing-artifact' }],
  ]
  for (const queries of variants) {
    ...
    const result = await check(root)
    assert.equal(result.status, 'passed_static')
    assert.equal(codes(result).has('trace.schema-invalid'), false)
    assert.equal(codes(result).has('trace.identity-dangling'), false)
  }
})
