...
1. 先确定问题、数据范围和要表达的结论。直接分析文件时使用 `dsh-data-analysis-files`；涉及 Marivo 分析语义或建模时，按需使用 `marivo-analysis`、`marivo-semantic` 与 `marivo_help`。通过 `marivo_python` 执行：纯本地 pandas／原生 DuckDB 使用 `datasources: []`，访问 Marivo datasource 才声明其精确名称。已有结果可直接展示，无需先激活分析 Skill。此 Skill 只定义展示，不代替 Marivo 的分析、质量与 lineage 契约。
2. 选数据。已有可恢复 Artifact 用 owning `sessionId` + `artifactRef` 引用，分别取实际 `session.id` 和 `artifact.ref`，不要从 Session 名称或日志标题猜测；必要时加 `findingId`，不重复导出同一份数据。需要计算后的 DataFrame 时，在绑定 Runtime 的 Python 中用 `dsh_data_analysis_presentation.write_dataset(frame, path)` 写纯 JSON；中文报告通过 `labels={列名: 中文展示名}` 设置表头、图例和 tooltip，Draft 字段绑定仍使用原列名，未映射列沿用列名。computed 的来源只在草稿声明，可为零个或多个 Artifact；文件分析在正文说明文件与读取范围，关联实际 `codeRef`，不构造 Artifact 来源。不要求转换代码、字段映射、hash 或输入输出证明。来源 Quality/issues 仍属于原 Artifact，不证明 computed 的计算正确。
