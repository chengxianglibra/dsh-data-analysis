...
分析中发现可供后续任务复用的业务定义或语义缺口时，先区分共享口径与本次结果：指标定义、维度含义或业务关系可作为语义候选；观测数值、临时筛选与未验证假设保留在当前分析中，不自动成为共享定义。

仅当当前问题需要该共享定义，或用户已授权语义沉淀时，加载 `marivo-semantic`，携带复用用途、已有依据、适用范围和未决含义，按其 Skill 与实时 Help 检查并优先复用已有对象。需要新增或修正时只处理最小必要缺口；业务含义与授权已明确则继续，实质歧义按语义 Skill 澄清，不自行猜测定义，也不重复请求已有授权。

收到经验证的对象引用后继续原问题；必要对象已完成且原问题已回答，或阻塞已明确时停止。不递归扩展相邻指标、实体或业务域。顺带发现的范围外机会只在答复中简要说明用途与依据，不自动建模、另建记录文件或继续探索。普通文件计算仍无需语义建模；展示流程不承担语义沉淀。
