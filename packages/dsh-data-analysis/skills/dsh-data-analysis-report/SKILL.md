...
4. Native/both 模式在所有资源完成后，以顶层文件 Tool 对 `index.html` 做 bundle 最后一次 mutation；资源再变化时须再次 mutation 入口。最终回答以 Markdown 行内代码逐字复制该调用使用的完整绝对 `file_path`，不得缩短为相对路径、basename、`file://` 或 Markdown 链接。Produced Files 与 Host opening 只是导航，不代表发布、不可变、可恢复或验证通过。
