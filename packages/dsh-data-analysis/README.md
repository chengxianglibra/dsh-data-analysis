...
会话标题旁会出现“语义层”和“数据源与凭证”入口。本文的操作说明以 Web 界面为例。
...
这里展示已有定义；浏览页面不会执行分析查询。两个管理面板都支持切换工作区。
