...
- 已安装并配置好 DeepSeek Harness `>=0.1.5-rc.1`（rc.1 验收状态见验证指南；范围内未来版本不视为已验收）。
...
更新插件后，如果旧浏览器页面中工具正在等待、却没有打开配置 Tab，也没有“待配置”入口，
先刷新浏览器页面以加载当前客户端。仍存活的原请求会重新显示；刷新页面不等于重启 Host。
若原调用已取消或 Host 已重启，需要重新发起配置请求。
...
- **继续追问**：在支持的报告内容菜单中选择“加入提问”，在输入框插入 `# <cell名称>` 引用标签，补充问题后发送；完整上下文会随引用一起提交。
